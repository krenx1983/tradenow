...
### **股票兼容接口：**
除提供开放平台模拟交易外，还提供使用CTP接口接入证券柜台的能力，同样使用CTP接口完全兼容的动态库对证券柜台接口进行封装，使得CTP程序无需任何修改，只更改CTP动态库即可接入证券柜台，进行股票、债券、逆回购、新股申购、融资融券、ETF期权等交易，目前已完成华鑫证券股票交易接入功能，同样发布的目前了在用四个CTPAPI版本，提供win32、win64、linux三套动态库。
